...
class ChatViewController: MessagesViewController, NVActivityIndicatorViewable,MessageCellDelegate {
    ...
    func showAlert(_ error: AssistantError) {
        ...
        DispatchQueue.main.async {
            ...
            if self.presentedViewController == nil {
                ...
                alert.addAction(UIAlertAction(title: "Sair", style: UIAlertAction.Style.default, handler: nil))
                // Show the alert
...
extension ChatViewController: MessagesLayoutDelegate {
    ...
    private func cellTopLabelAlignment(for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) -> LabelAlignment {
        if isFromCurrentSender(message: message) {
            return LabelAlignment.init(textAlignment: NSTextAlignment(rawValue: Int(0))!,
                                       textInsets: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 10))
            //.messageTrailing(UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 10))
        } else {
            return LabelAlignment.init(textAlignment: NSTextAlignment(rawValue: Int(0))!,
                                       textInsets: UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 0))
...
extension ChatViewController: MessageInputBarDelegate {
    ...
    func messageInputBar(_ inputBar: MessageInputBar, didPressSendButtonWith text: String) {
        ...
        func showMapScreen(){
            guard let mainVC = storyboard?.instantiateViewController(withIdentifier: "MapViewController") else{return}
        ...
        assist.message(workspaceID: workspace, request: messageRequest, failure: failAssistantWithError) { response in
            ...
            for watsonMessage in response.output.text {
                ...
                DispatchQueue.main.async {
                    print(watsonMessage)
                    if watsonMessage == "Certo, estamos redirecionando..."{
                        print("Adauto seu filho da puta")
                        showMapScreen()
